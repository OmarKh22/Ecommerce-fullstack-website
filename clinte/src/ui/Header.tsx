        ...
        <Container className="py-2 max-w-4xl flex items-center gap-5 justify-between">
          <Menu>
            ...
            >
              <MenuItems
                anchor="bottom end"
                className="ml-5 w-52 origin-top-right rounded-xl border border-white/5 bg-darkText text-whiteText p-1 text-sm/6 text-gray-300 [--anchor-gap:var(--spacing-1)] focus:outline-none hover:text-white z-50 "
              >
                {categories.map((item: CategoryProps) => (
                  <MenuItem key={item?._id}>
                    <Link
                      to={`/category/${item?._base}`}
                      className="flex w-full items-center gap-2 rounded-lg py-2 px-3 data-[focus]:bg-white/20 tracking-wide hover:bg-whiteText/20"
                    >
                      <img
                        src={item?.image}
                        alt="categoryImage"
          ...
          {bottomNavigation.map(({ title, link }) => (
            <Link
              to={link}
              key={title}
              className="uppercase hidden md:inline-flex text-sm font-semibold text-whiteText/90 hover:text-whiteText duration-200 relative overflow-hidden group hover:cursor-pointer"
            >
              {title}
